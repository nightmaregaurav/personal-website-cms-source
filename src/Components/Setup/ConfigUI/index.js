...
const ConfigUI = ({onChange, isGhPage, info, name, parent_disabledStatus=false}) => {
    ...
    switch (type) {
        ...
        case 'IMAGE URL':
            return <ImageUrlUI onChange={onChange} isGhPage={isGhPage} info={info} name={name} parent_disabledStatus={parent_disabledStatus} />;
        case 'OBJECT':
            return <ObjectUI onChange={onChange} isGhPage={isGhPage} info={info} name={name} parent_disabledStatus={parent_disabledStatus} />;
        default:
            if(type.startsWith('ARRAY OF ')) {
                const sub_type = type.substring(9);
                return <ArrayUI elementType={sub_type} onChange={onChange} isGhPage={isGhPage} info={info} name={name} parent_disabledStatus={parent_disabledStatus} />;
            } else {
